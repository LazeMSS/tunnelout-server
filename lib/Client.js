...
class Client extends EventEmitter {
    constructor(options) {
        ...
        this.authusr = options.authusr;
        this.authpass = options.authpass;
        this.ipAdr = options.ipAdr;
        ...
        // TODO(roman): an agent error removes the client, the client needs to re-connect?
        // how does a client realize they need to re-connect vs some random client being assigned same port?
    ...
    stats() {
        let stats = this.agent.stats();
        stats['last10request'] = this.last10request;
        stats['lastActivity'] = this.lastActivity;
        return stats;
    }
    ...
    getAuthInfo() {
        if (this.authusr == null || this.authpass == null) {
            return null;
        } else {
            return { usr: this.authusr, pass: this.authpass };
        }
    }
    ...
    handleRequest(req, res) {
        ...
        let requestLog = { url: req.url, ip: ip, reqTime: Date.now(), method: req.method, headers: req.headers };
        ...
        clientReq.once('error', (err) => {
            if (err.code === 'ECONNRESET' || err.code === 'ETIMEDOUT' || err.message === 'closed') {
                return;
            }
            console.error(err);
