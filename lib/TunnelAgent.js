...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        ...
        if (options.secure) {
            let secureoptions = {
                key: fs.readFileSync(options.keyFile, 'ascii'),
                cert: fs.readFileSync(options.certFile, 'ascii')
            };

            this.debug('Creating a secure tunnel!');
            this.server = tls.createServer(secureoptions);
        } else {
            this.debug('Creating a insecure tunnel!');
    ...
    createConnection(options, cb) {
        ...
        // socket is a tcp connection back to the client hosting the site
    ...
    destroy() {
        ...
        this.server.close();
        // Close each socket
        for (let key in this.availableSockets) {
            this.debug('destroying :' + key);
