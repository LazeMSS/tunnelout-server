...
class ClientManager {
    constructor(opt) {
        ...
        */
        ...
        // OPTIMIZATION: Read certs once at startup if secure
        if (this.opt.secure) {
            try {
                this.opt.keyData = fs.readFileSync(this.opt.keyFile, 'ascii');
                this.opt.certData = fs.readFileSync(this.opt.certFile, 'ascii');
            } catch (err) {
                console.error('Could not load SSL certs in ClientManager:', err);
            }
        }
    ...
    async newClient(id, authusr = null, authpass = null, ipAdr = null, agentName = null) {
        ...
        // Pass pre-loaded cert data to agent (prevents disk read)
        if (this.opt.secure) {
            agentOpts.keyData = this.opt.keyData;
            agentOpts.certData = this.opt.certData;
        }
