...
export default function (opt) {
    ...
    async function getClientHostnameFromClientsList(clientID) {
    ...
    async function loadClients() {
        ...
        // Do we have the file
        try {
            await fsPromises.access(clientsFile, fs.constants.R_OK);
        } catch (e) {
             console.error('"%s" file not found or unreadable', clientsFile);
             return false;
        }

        //try read the file
        try {
            let data = await fsPromises.readFile(clientsFile);
    ...
    async function writeClients() {
        if (!doWeHaveClientsList()) return;
        try {
            let data = JSON.stringify(clientsList, null, 2);
            await fsPromises.writeFile(clientsFile, data);
        } catch (err) {
    ...
    async function checkClientHeaderLogin(ctx) {
        let clientHKey = null;
        ...
        let clientHost = await getClientHostnameFromClientsList(clientHKey);
        if (clientHost == null) {
    ...
    async function customErrorWeb(ctx, code) {
        ctx.status = code;
        await webserveFile(ctx, code + '.html');
    }

    async function htmlReplacer(inputFilename) {
        try {
            const content = await fsPromises.readFile(inputFilename, 'utf-8');
            return content
            ...
            console.error('Error in htmlReplacer:', err);
            return '';
        }
    }
    ...
    /* basic webserver (Optimized with Async I/O) */
    async function webserveFile(ctx, filename) {
        ...
        try {
            ...
            // Prevent infinite loop if 404.html is missing
            if (filename.endsWith('404.html')) {
                ctx.body = '404 Not Found';
                return;
            }
            await customErrorWeb(ctx, 404);
            return;
        }
        ...
        if (fileExt == 'html' || fileExt == 'htm') {
            ctx.body = await htmlReplacer(filename);
        } else {
    ...
    function buildAuthRequest(ctx) {
        ...
        // We cannot call customErrorWeb sync here, so we set status and 
        // rely on the caller or the error wrapper to handle the HTML response.
        // The original code called customErrorWeb(ctx, 401) here, but since that is now async,
        // we will let the caller handle the await.
        ctx.status = 401; 
        return false;
    }
    ...
    function adminAuthCheck(ctx, promptLogin, headers = null) {
        ...
        if (!('authorization' in headers)) {
            if (ctx != null && promptLogin) {
                ...
                // Trigger the HTML page for 401
                customErrorWeb(ctx, 401).catch(e => console.error(e));
            }
    ...
    // Auth a basic client
    async function clientAuth(client, ctx) {
    ...
    router.get('/favicon.ico', async (ctx) => {
        await webserveFile(ctx, 'favicon.ico');
    });
    ...
    router.get(dashPath + '/c/:clientid/(.*)', async (ctx) => {
        ...
        if (!(await clientAuth(client, ctx))) {
            return;
        }
    ...
    router.get('/api/status', async (ctx) => {
        ...
        if (os.platform() == 'linux') {
            try {
                // Async read for meminfo
                const memData = await fsPromises.readFile('/proc/meminfo', 'utf8');
                availMem = Math.floor(Number(/MemAvailable:[ ]+(\d+)/.exec(memData)[1]) / 1024) + ' MB';
            } catch (e) { }
        }
    ...
    router.get('/api/tunnels/:id', async (ctx) => {
        ...
        if (!apiKeyCheck(ctx.request.headers) && !(await clientAuth(client, ctx))) {
            return false;
        }
    ...
    app.use(async (ctx, next) => {
        ...
        let cKey = null;
        if ('x-client-key' in ctx.request.headers && ctx.request.headers['x-client-key'] != undefined) {
            clientReqHostName = await getClientHostnameFromClientsList(ctx.request.headers['x-client-key']);
    ...
    loadClients().catch(e => console.error('Initial clients load failed:', e));
    ...
    server.on('request', async (req, res) => {
        ...
        if (!clientId) {
            ...
            if (req.url.indexOf('/api/clients/') == 0 && req.method == 'POST' && req.headers['content-type'].indexOf('application/json') == 0) {
                ...
                if (!doWeHaveClientsList()) {
                    res.statusCode = 404;
                    // Async html replacer for 404
                    res.end(await htmlReplacer(dashFolder + '404.html'));
                    return;
                }
        ...
        if (authData != null) {
            ...
            if (!('authorization' in req.headers) || authThis(req.headers['authorization'], authData.usr, authData.pass) == false) {
                ...
                // Async html replacer for 401
                res.end(await htmlReplacer(dashFolder + '401.html'));
                return;
            }
        }
